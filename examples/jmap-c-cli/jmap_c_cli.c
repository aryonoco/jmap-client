...
static int die(const char *what, jmap_status s) {
  fprintf(stderr, "%s: %s (%s)\n", what, g_client ? jmap_errmsg(g_client) : "-",
          jmap_strerror(s));
  return 1;
}
...
static int connect_from_env(void) {
  ...
  if (s != JMAP_OK)
    return die("connect", s);
  s = jmap_client_primary_account(g_client, &g_account);
  if (s != JMAP_OK)
    return die("primary account", s);
  return 0;
}
...
static int cmd_search(const char *text) {
  ...
  if (s != JMAP_OK)
    return die_status("query new", s);
  s = jmap_query_set_str(q, JMAP_Q_TEXT, text);
  if (s == JMAP_OK)
    s = jmap_query_set_u32(q, JMAP_Q_LIMIT, 20);
  if (s == JMAP_OK)
...
static int cmd_move(const char *id, const char *mailbox_id) {
  const char *ids[] = {id};
  jmap_set_result *r = NULL;
  jmap_status s = jmap_move_emails(g_client, g_account, ids, 1, mailbox_id, &r);
  if (s != JMAP_OK)
    return die("Email/set", s);
...
static int cmd_sync(const char *maybe_state) {
  ...
  if (s != JMAP_OK) {
    ...
    if (type)
      fprintf(stderr, "sync: method error %s\n", type);
    return die("sync", s);
  }
...
static int find_role_mailbox(jmap_mailbox_role role, char *out, size_t cap) {
  ...
  jmap_mailboxes_free(mbs);
  if (!found)
    fprintf(stderr, "no mailbox with the required role\n");
  return rc;
}
...
static int cmd_send(const char *to, const char *subject, const char *body) {
  ...
  failed = copy_borrow(identity_id, sizeof identity_id, jmap_identity_id(ident),
                       "identity id");
  if (!failed)
  ...
  if (failed)
    return 1;
  if (find_role_mailbox(JMAP_ROLE_DRAFTS, drafts, sizeof drafts))
    return 1;
  if (find_role_mailbox(JMAP_ROLE_SENT, sent, sizeof sent))
    return 1;
  ...
  if (s == JMAP_OK)
    s = jmap_message_set_str(m, JMAP_MSG_DRAFTS_MAILBOX, drafts);
  if (s == JMAP_OK)
    s = jmap_message_set_str(m, JMAP_MSG_SENT_MAILBOX, sent);
  if (s == JMAP_OK)
    s = jmap_message_set_str(m, JMAP_MSG_FROM, from_addr);
  if (s == JMAP_OK)
    s = jmap_message_set_str(m, JMAP_MSG_TO, to);
  if (s == JMAP_OK)
    s = jmap_message_set_str(m, JMAP_MSG_SUBJECT, subject);
  if (s == JMAP_OK)
    s = jmap_message_set_str(m, JMAP_MSG_BODY, body);
  if (s != JMAP_OK) {
    jmap_message_free(m);
    return die_status("message", s);
  }
  ...
  if (s != JMAP_OK) {
    ...
    if (s == JMAP_E_SET) {
      ...
      if (type)
        fprintf(stderr, "send: refused with %s\n", type);
    }
    return die("send", s);
  }
...
static int cmd_vacation(jmap_vacation_state vs, const char *subject,
  ...
  jmap_vacation_update_free(u);
  if (s != JMAP_OK)
    return die("VacationResponse/set", s);
  return print_set_result(r, "vacation updated");
}

static int usage(void) {
  fprintf(stderr, "usage: jmap-c-cli mailboxes | search <text> | read <id> |\n"
...
static command parse_command(int argc, char **argv) {
  ...
  if (strcmp(verb, "mailboxes") == 0 && argc == 2)
    return CMD_MAILBOXES;
  if (strcmp(verb, "search") == 0 && argc == 3)
    return CMD_SEARCH;
  if (strcmp(verb, "read") == 0 && argc == 3)
    return CMD_READ;
  if (strcmp(verb, "flag") == 0 && argc == 3)
    return CMD_FLAG;
  if (strcmp(verb, "move") == 0 && argc == 4)
    return CMD_MOVE;
  if (strcmp(verb, "sync") == 0 && argc <= 3)
    return CMD_SYNC;
  if (strcmp(verb, "send") == 0 && argc == 5)
    return CMD_SEND;
  if (strcmp(verb, "vacation") == 0 && argc >= 3 && argc <= 5) {
    if (strcmp(argv[2], "on") == 0)
      return CMD_VACATION_ON;
    if (strcmp(argv[2], "off") == 0)
      return CMD_VACATION_OFF;
  }
  return CMD_NONE;
}
...
static int dispatch(command cmd, int argc, char **argv) {
  ...
  case CMD_FLAG:
    return cmd_flag(argv[2]);
  case CMD_MOVE:
    return cmd_move(argv[2], argv[3]);
  case CMD_SYNC:
    return cmd_sync(argc == 3 ? argv[2] : NULL);
  case CMD_SEND:
    return cmd_send(argv[2], argv[3], argv[4]);
