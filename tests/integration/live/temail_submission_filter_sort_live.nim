...
## ``EmailSubmissionComparator`` against a real multi-submission
## corpus.  Visibly-harder capstone (mirrors A7 / B12 / C18 / D24
## / E30 / F36 / G42 / H48): builds two identities, four
## submissions, and chains ``/query`` → ``/queryChanges`` from a
## captured baseline.
##
## Workflow:
##
...
##  4. ``seedSubmissionCorpus`` with the two identities, alice→bob
##     recipient, and four distinct subject suffixes.  Polls each
##     to ``usFinal``.
##  5. Sub-test A: filter ``identityIds = [primary]`` — assert at
##     least the two primary-identity submissions surface.
##  6. Sub-test B: sort by ``sentAt`` ascending — assert all four
##     surface and the ordering is non-decreasing.  Capture the
##     wire response.
##  7. Sub-test C: ``EmailSubmission/queryChanges`` from the
##     baseline with ``calculateTotal: true``.  Capture the wire
##     response; assert total advanced by at least four.
##
...
block temailSubmissionFilterSortLive:
  ...
  if cfgRes.isOk:
    ...
    let submissionIds = seedSubmissionCorpus(
        ...
        drafts,
        aliceAddr,
        identities = @[primaryId, secondaryId, primaryId, secondaryId],
        recipients = @[bobAddr],
        subjects = @[
          "phase-i 60 sub-a", "phase-i 60 sub-b", "phase-i 60 sub-c", "phase-i 60 sub-d"
        ],
        creationLabelPrefix = "phase-i-60",
      )
      .expect("seedSubmissionCorpus")
    doAssert submissionIds.len == 4,
      "four submissions expected (got " & $submissionIds.len & ")"
    let primarySubmissions = @[submissionIds[0], submissionIds[2]]
    let secondarySubmissions = @[submissionIds[1], submissionIds[3]]
    ...
    doAssert int64(qcr.total.unsafeGet) >= 4,
      "total must reflect at least the four new submissions (got " & $qcr.total.unsafeGet &
        ")"
