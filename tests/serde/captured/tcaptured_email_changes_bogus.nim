...
## Parser-only replay test for the captured ``Email/changes`` bogus-
## ``sinceState`` method-level error (RFC 8620 §5.2 / §5.5,
## ``tests/testdata/captured/email-changes-bogus-state-stalwart.json``).
...
## currently picks ``invalidArguments``.  The assertion uses
## set-membership so a future Stalwart upgrade onto either RFC-compliant
