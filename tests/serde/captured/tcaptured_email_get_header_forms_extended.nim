...
block tcapturedEmailGetHeaderFormsExtended:
  let j = loadCapturedFixture("email-get-header-forms-extended-stalwart")
  ...
  let resentAllKey = parseHeaderPropertyName("header:Resent-To:asAddresses:all").expect(
      "parseHeaderPropertyName resentAll"
    )
