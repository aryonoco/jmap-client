...
## response with a name filter and ``calculateTotal: true``
## (``tests/testdata/captured/mailbox-query-changes-with-filter-stalwart.json``).
## Verifies that ``QueryChangesResponse[Mailbox]`` parses the RFC 8620
## §5.6 fields — ``oldQueryState`` / ``newQueryState`` / ``total`` /
## ``removed`` / ``added`` — when filter is present in the original
...
testCase tcapturedMailboxQueryChangesWithFilter:
  let j = loadCapturedFixture("mailbox-query-changes-with-filter-stalwart")
