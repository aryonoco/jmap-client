...
## ``envelope.Response`` (used by ``send``), or a ``JmapError`` carrying
## the appropriate transport / request / protocol failure.
##
...
proc tryParseProblemDetails(body: string): Opt[JmapError] =
  ## Attempts to parse RFC 7807 problem details from a response body.
  ## Returns ``Opt.some(JmapError)`` (a ``jeRequest`` rejection) on success,
  ## ``none`` when the body is not JSON, not an object, missing ``"type"``,
  ## or otherwise fails ``RequestError.fromJson``.
  try:
    {.cast(raises: [CatchableError]).}:
      ...
      if jsonNode.kind == JObject and jsonNode.hasKey("type"):
        ...
        if reqErrResult.isOk:
          return Opt.some(jmapRequest(reqErrResult.get()))
  except CatchableError:
    discard
  Opt.none(JmapError)

proc parseJsonBody(body: string, context: RequestContext): Result[JsonNode, JmapError] =
...
proc classifyStatus(
  ...
  ## problem-details extraction (``jeRequest``) then fall back to a generic
  ## ``httpStatusError`` (``jeTransport``); anything else (1xx, 3xx) is rejected.
  if statusCode >= 400 and statusCode < 600:
    if contentType.startsWith("application/problem+json") or
        contentType.startsWith("application/json"):
      for je in tryParseProblemDetails(body):
        return err(je)
    return err(
      jmapTransport(
...
proc parseJmapResponse*(
    httpResp: HttpResponse, context: RequestContext
): Result[envelope.Response, JmapError] =
  ## Classifies the HTTP response, parses the body as JSON, detects
  ## RFC 7807 problem details on HTTP 200 (``jeRequest``), and decodes the
  ## envelope (a decode failure becomes an unlocalised ``jeProtocol`` — there
  ## is no single method call to localise the whole-envelope fault to). Used
  ## by ``send`` and by tests that POST adversarial bodies through their own
  ## transport.
  ...
  if respJson.kind == JObject and respJson.hasKey("type") and
    ...
    if reqErrResult.isOk:
      return err(jmapRequest(reqErrResult.get()))
