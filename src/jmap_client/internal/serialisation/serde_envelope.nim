...
## L2-internal canonical envelope SerDe (RFC 8620 §3.2-3.4, 3.7):
## ``Invocation``, ``Request``, ``Response``, ``ResultReference``.
##
## Application-facing wire-shape diagnostics are NOT here.
## The supported send-side diagnostic is ``BuiltRequest.toJson``
## (``internal/protocol/builder.nim``), modelled after SQLite's
## ``sqlite3_expanded_sql(stmt)``. The receive-side diagnostic is
## the per-handle ``setDebugCallback`` on ``JmapClient``,
## modelled after libcurl's ``CURLOPT_DEBUGFUNCTION``. This
## module retains ``*`` on ``Invocation.toJson``,
## ``Request.toJson``, and ``ResultReference.toJson`` for cross-
## internal use; the hub filter at ``internal/protocol.nim``
## keeps them out of ``import jmap_client``. ``Response.toJson``
## is deleted.
...
func toJson*(inv: Invocation): JsonNode =
  ...
  ## symbol name. Hub-private after A16 (filtered out of
  ## ``internal/protocol.nim``); reachable to in-tree callers under
  ## direct H10 import.
  return %*[inv.rawName, inv.arguments, $inv.methodCallId]

func toJson*(r: Request): JsonNode =
  ## Serialise Request to JSON (RFC 8620 section 3.3). Hub-private
  ## after A16; reachable to ``client.performSend`` (HTTP-body
  ## construction) and to ``BuiltRequest.toJson`` (the application-
  ## facing diagnostic seam).
  var node = newJObject()
  node["using"] = %r.`using`
  var calls = newJArray()
...
func toJson*(r: ResultReference): JsonNode =
  ## Serialise ResultReference to JSON (RFC 8620 section 3.7). Hub-
  ## private after A16; reachable to ``methods.nim``'s back-reference
  ## emission paths (``GetRequest`` ``ids`` and ``SetRequest``
  ## ``destroy`` rkReference arms). Uses ``rawName`` / ``rawPath`` to
  ## preserve verbatim wire strings, including any forward-compatible
  ## unknown variants.
...
func fromJson*(
  ...
  return wrapInner(parseRequest(usingSeq, methodCalls, createdIds), path)
...
func fromJson*(
  ...
  let createdIds = ?parseCreatedIds(node, path)
  return ok(initResponse(methodResponses, createdIds, sessionState))
