...
- The 18 L5 handle/view object types (Tasks 2, 3, 6–13) carry
...
- Produces: Nim-private `JmapSendResultHandle{emailId, submissionId:
  string}`, `func anyNil` (the shared NULL sweep over a string argument
  list), `func sendTargets` (the three server-issued ids parsed as one
  unit); C functions `jmap_send_plain_text`,
...
func anyNil(values: openArray[cstring]): bool =
  ## One NULL sweep for a call that takes seven strings: the C contract
  ## is identical for each, so the argument list is data here rather than
  ## a seven-term condition in the export.
  for v in values:
    if v.isNil:
      return true
  false
...
func sendTargets(
    identityRaw, draftsRaw, sentRaw: cstring
): Result[tuple[identity: Id, mailboxes: SendMailboxes], JmapError] =
  ## Parses the three server-issued ids a send names, in one place: the
  ## boundary yields either the whole target or the first reason it
  ## cannot, which is what the one-shot's argument list wants.
  let identity = ?parseIdFromServer($identityRaw).lift
  let drafts = ?parseIdFromServer($draftsRaw).lift
  let sent = ?parseIdFromServer($sentRaw).lift
  ok((identity: identity, mailboxes: SendMailboxes(drafts: drafts, sent: sent)))
...
proc jmapSendPlainText(
  ...
  ## §7.5.1), so this parses the arguments and records the outcome.
  if not l5Initialised:
    return asCint(jsMisuse)
  if client.isNil:
    return asCint(jsMisuse)
  if outResult.isNil:
    return recordMisuse(client, "out parameter must not be NULL")
  if anyNil(
    [identityId, draftsMailboxId, sentMailboxId, fromAddr, toAddr, subject, body]
  ):
  ...
  if parsedAcct != asCint(jsOk):
    return parsedAcct
  let targets = sendTargets(identityId, draftsMailboxId, sentMailboxId)
  if targets.isErr:
    return recordError(client, targets.error)
  let resp = sendPlainText(
    client[].client, acct, targets.get().identity, targets.get().mailboxes,
    PlainTextMessage(
