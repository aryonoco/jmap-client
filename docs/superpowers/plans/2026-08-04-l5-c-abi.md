...
- Produces: Nim-private `SetFailureItem`, `JmapSetResultHandle{updated,
  ...
  toSetResultContent[T, U](resp: SetResponse[T, U]):
  JmapSetResultHandle`; C functions
...
func toSetResultContent[T, U](resp: SetResponse[T, U]): JmapSetResultHandle =
  ## Updated ids echo no server-generated fields back for the verbs this
  ## view serves, so only the id is worth keeping. Generic over both
  ## payload parameters: `T` types the create payload alone, so an entity
  ## that cannot be created still has rows on the update and destroy
  ## rails.
...
- Produces: C enums `jmap_query_opt`, `jmap_read_state`, `jmap_sort`;
  ...
  `QueryParams`), `func accumulated`, `proc setQueryLimit`, `proc
  setQueryReadState`, `proc setQuerySort`, `func querySpec`.
...
proc setQueryLimit(query: ptr JmapQueryHandle, value: uint32): cint =
  ...
  ## whole-object replacement would silently reset `position`, `anchor`,
  ## `anchorOffset` and `calculateTotal` too.
  if value == 0'u32:
