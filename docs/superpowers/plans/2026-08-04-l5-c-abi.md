...
- **Settled decisions binding this plan** (owner rulings, 2026-08-04):
  ...
  4. Send is a MESSAGE HANDLE plus enum-dispatched typed setters
     (`jmap_message_new` / `jmap_message_set_str` / `jmap_message_free`,
     then `jmap_send`), not the wide `jmap_send_plain_text` the §8 map
     sketches — eight consecutive `const char *` let a caller transpose
     drafts with sent, or from with to, undetectably, and a fixed arity
     cannot grow additively (Task 17 records the dated amendment).
...
- Produces: C opaque handles `jmap_message` and `jmap_send_result`; the
  ...
  `jmap_message_set_str`, `jmap_message_free`, `jmap_send`,
  `jmap_send_result_free`, `jmap_send_result_email_id`,
  `jmap_send_result_submission_id`; Nim-private `JmapMessageHandle`,
  `JmapSendResultHandle`, `func recipientList`, `func sendArguments`,
  `proc setMessageId`, `proc setMessageText`.
 ...
 * brace that closes that key's value. Every recipient also appears in
 * the submission envelope's rcptTo union, so a bare presence check
 * would pass a setter wired to the wrong role; this binds an address
 * to its own header without parsing JSON. */
...
int main(void) {
  ...
  /* An option this build does not know is refused, never a no-op. */
  assert(jmap_message_set_str(m, (jmap_message_opt)99, "x")
         == JMAP_E_MISUSE);
  ...
  /* Every value a setter took reached the wire, in the role it was set
   * for, and the value set SECOND is the one that travelled. */
...
`"bcc"` and `"subject"`, which is what `role_carries` keys off. If the
serialised spacing or member order defeats it, re-anchor each address to
its own role by whatever stable spelling the emitted JSON does have —
never weaken to a bare presence check, which a setter wired to the wrong
slot would sail straight through.
...
typedef enum {
  ...
  JMAP_MSG_TO             = 4, /* string: one To recipient */
  JMAP_MSG_CC             = 5, /* string: one Cc recipient */
  JMAP_MSG_BCC            = 6, /* string: one Bcc recipient */
 ...
 * earlier value outright: no option accumulates, so JMAP_MSG_TO,
 * JMAP_MSG_CC and JMAP_MSG_BCC each carry exactly one address.
 *
...
type JmapMessageHandle {.ruleOff: "objects".} = object
  ...
  ## Subject and body need no such distinction — absent and empty travel
  ## identically — so they are plain strings with one representation.
  identity: Opt[Id]
  drafts: Opt[Id]
  sent: Opt[Id]
  fromAddr: Opt[string]
...
func recipientList(slot: Opt[string]): seq[string] =
  ## One address per role. A slot the caller never named contributes no
  ## header and no envelope recipient; a slot named twice carries the
  ## second value alone, because the setter assigns rather than
  ## accumulates.
  for address in slot:
    return @[address]
  @[]
...
proc setMessageText(
  ...
  ## set twice keeps the second value only.
  let text = $value
  case opt
...
all six runs in the task report:
...
- [ ] **Step 1: Dated amendments in doc 17** (match the file's voice;
  ...
  - §8: the operation map's "Send plain text" row names
    ...
    (`jmap_message_new` / `jmap_message_set_str` / `jmap_message_free`,
    then `jmap_send`) — the same setopt discipline §5 now carries, for
