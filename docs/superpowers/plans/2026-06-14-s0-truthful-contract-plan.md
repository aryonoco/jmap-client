...
- [x] Phase 3 — type-shape rendering → `--mode:type-shapes` body — done: 260 sections (≥ old 242; +18 types, 0 dropped); private `raw*` excluded via `sfExported`; enum wire strings; case scaffolding; proc-type/alias RHS captured (SendProc/CloseProc/DebugCallback/JmapResult/ParseProc); sealed handles empty; deterministic. Run: `API_ORACLE_MODE=type-shapes /tmp/api_oracle check --mm:arc --threads:on --panics:on --path:src --path:vendor/nim-results scripts/api_probe.nim`. See git log.
- [x] Phase 4+5 — rewire freeze recipes + H16/H17 lints to the oracle, retire scraper, regenerate baseline, negative-control proof — landed together as ONE atomic commit (mutually dependent: regenerating the baseline with the oracle while lints used the scraper, or deleting api_surface.nim that the lints import, would break `just ci`; never commit a red tree). Diff: public-api +1326/-297, type-shapes +138; adversarial audit = no-regression (the lone removal `fromJson(MailboxChangesResponse)` is a correctness win — oracle honours `export types except fromJson`); gensym suffixes stripped; round-trip + negative control + determinism all pass. See git log.
- [~] Phase 6 — docstrings, full ci, PR — VERIFIED, awaiting PR: no stale "1.0 contract" framing (lints say "consumer-facing contract"); **`just ci` GREEN** (reuse + fmt-check + lint + lint-isolated + lint-style + all H-lints + lint-public-api + lint-type-shapes + analyse + test — CI_EXIT=0); **`just clean && just jmap-reset && just test-full` GREEN** (TESTFULL_EXIT=0, "All shards passed", 0 fail/0 skip, live-Stalwart integration executed). REMAINING ONLY: `git push -u origin api/s0-truthful-contract` + open PR (outward — confirming with user first).

  **Incident/lesson (resolved):** the Phase 4+5 commit used `git add -A`, which swept two pre-existing UNTRACKED, unrelated files (`examples/jmap-cli/.gitignore`, `examples/jmap-cli/config.nims`) into the S0 commit. They were also the only `just reuse` failures (no SPDX header; `.nims`/example-`.gitignore` not covered by a REUSE.toml glob). Per the user they were not needed (untracked nimble cruft; the example builds via `nim.cfg`), so deleted, and `git commit --amend` removed them from the S0 commit. **Going forward: stage explicit paths, never `git add -A`.** (Pre-existing tracked `examples/jmap-cli/{jmap_cli.nimble,nimble.paths}` are an unrelated latent matter, left untouched — out of S0 scope.)
