...
/*
 * jmap_client is the C API of the jmap-client library.
 *
 * Call jmap_init() once per process. Only jmap_strerror() and
 * jmap_version() work before that call.
 *
 ...
 * alone. jmap_errmsg() gives readable text for the last error on a
 * client.
 *
 * This library owns everything its read accessors return. A returned
 * const char *, and a returned pointer to an object, is a borrow and is
 * valid until the handle that produced it is freed. Do not free a
 * borrow. Objects are released only by their paired jmap_*_free
 * function. The borrows from jmap_errmsg(), jmap_errtype() and
 * jmap_get_email_state() have a shorter life. The next call that can
 * fail on the same client replaces them.
 *
 * Use a handle from one thread at a time. To move a handle to another
 * thread, stop using it on the old thread first.
 *
 * This header is the C ABI and may still change (prior to 1.0). But
 * breaking changes will be rare and will be avoided if possible.
 */
...
/*
 * JMAP_E_METHOD means the server answered the method call with an
 * error instead of a result. Call jmap_errtype() to read the error
 * type string the server sent. RFC 8620 section 3.6.2 describes these
 * errors.
 *
 ...
 * section 5.3). jmap_send() is the call in this header that reports a
 * refusal this way. Call jmap_errtype() for that reason too.
 *
 ...
 * different thing in each. So the status is not a convenience. Read it
 * before you compare the string, or you cannot tell which of the two
 * you are holding.
 */
 ...
 *
 * It is not thread-safe. Calling it again from the same thread is a
 * no-op.
 */
 ...
 *
 * Do not follow a jmap_cleanup() with a jmap_init(). Use the pair once
 * per process, in that order.
 */
void jmap_cleanup(void);

/*
 * jmap_strerror() returns a fixed description of a status code. The
 * string is never NULL. Do not free it. You may call it before
 * jmap_init().
 */
 ...
 * The string is fixed. Do not free it. You may call it before
 * jmap_init().
 */
 ...
 * first use, so a server you cannot reach shows up on the first
 * operation, not here.
 *
 * On failure *out is left alone and the returned status is the whole
 * report.
 */
jmap_status jmap_client_new(const char *session_url, const char *username,
...
/*
 * jmap_errmsg() returns readable text for the most recent error on
 * this client. It returns "no error" when the last call succeeded.
 *
 * The client owns the string. Do not free it. It is valid until the
 * next call that can fail on the same client, or until
 * jmap_client_free(). Copy it if you need it later.
 */
const char *jmap_errmsg(const jmap_client *client);

/*
 * jmap_errtype() returns the "type" string the server sent for the
 * last typed error on this client. Two statuses carry one.
 *
 * After JMAP_E_METHOD it is a method error type, for example
 * "unknownMethod". After JMAP_E_SET it is the reason the one create
 * was refused, for example "overQuota". The status says which list the
 * string came from, and the lists overlap, as the note on jmap_status
 * above explains.
 *
 * Use jmap_errmsg() for text a person reads and this function for a
 * value your code compares. The library carries the type off the wire
 * unchanged, so a type one vendor invented reaches C intact.
 * jmap_set_result_failure_type_at() works the same way, for refusals
 * that come back as data on a result instead of as a status.
 *
 ...
 * failure with any other status. When it is not NULL it is never the
 * empty string, so "no type" and "an empty type" never look alike.
 *
 * The borrow lives as long as the one from jmap_errmsg().
 */
const char *jmap_errtype(const jmap_client *client);

/* --- Bring your own HTTP ------------------------------------------ */

typedef enum { JMAP_HTTP_GET = 0, JMAP_HTTP_POST = 1 } jmap_http_method;
 ...
 * GET carries an empty body string rather than a NULL one, and they
 * are valid only while the call runs. Copy anything you need to keep.
 *
 ...
 * any other return value it ignores the out-parameters.
 *
 * Allocate both with the same C runtime this library links against. If
 * the two differ, the free() the library makes is undefined behaviour.
 *
 ...
 */
typedef jmap_transport_code (*jmap_send_fn)(
    void *userdata, jmap_http_method method, const char *url, const char *body,
    const char *authorization, int *out_http_status, char **out_content_type,
    char **out_body);
...
jmap_status jmap_transport_new(jmap_send_fn send, jmap_close_fn close,
                               void *userdata, jmap_transport **out);
...
/*
 * These functions fetch the JMAP session on first use and keep it, so
 * they take a client that is not const. Expect transport and protocol
 * statuses from that first call.
 *
 ...
 */
jmap_status jmap_client_primary_account(jmap_client *client, const char **out);

/*
 * jmap_client_account_count() writes the number of accounts to *out. It
 * is the only count here that reports through a status instead of
 * returning the number, because it is one of the calls that fetches the
 * session and so can fail. Every other _count reads a handle you
 * already hold and returns the number directly.
 */
 ...
 *
 * Every accessor here that takes an index returns NULL for an index
 * that is out of range. Any index not below the count is out of range,
 * including very large ones such as SIZE_MAX.
 */
const char *jmap_client_account_at(const jmap_client *client, size_t i);

/* --- Wire debugging ----------------------------------------------- */

typedef enum { JMAP_WIRE_SEND = 0, JMAP_WIRE_RECEIVE = 1 } jmap_wire_direction;

/*
 * jmap_debug_fn fires once per direction for each HTTP exchange the
 * client attempts, so at most twice per exchange.
 *
 * A send fire does not promise a receive fire after it. An oversized
 * request, a session URL it could not resolve, or a transport failure
 * can leave a send with no matching receive.
 *
 * bytes is never NULL, even when len is 0, so memcpy(dst, bytes, len)
 * is always well defined. It comes with a length and is not
 * NUL-terminated, so do not treat it as a C string. The library owns
 * it for the length of the call only. Copy it if you keep it.
 *
 ...
 * is still running. Freeing the client from inside it leaves the
 * library reading freed memory once it returns.
 */
typedef void (*jmap_debug_fn)(void *userdata, jmap_wire_direction direction,
 ...
 */
jmap_status jmap_set_debug_callback(jmap_client *client, jmap_debug_fn fn,
                                    void *userdata);
 ...
 * role. JMAP_ROLE_UNKNOWN means the server named a role this version
 * of the library does not know. Read the string the server sent with
 ...
 */
jmap_status jmap_get_mailboxes(jmap_client *client, const char *account_id,
                               jmap_mailboxes **out);
...
/* The _get suffix is here only so the function does not collide with
 * the jmap_mailbox_role type above. */
 ...
 * names no right the mailbox could hold. */
 ...
 */
jmap_status jmap_get_emails(jmap_client *client, const char *account_id,
                            const char *const *ids, size_t n,
                            jmap_emails **out);
/*
 * jmap_emails_free() releases a handle from jmap_get_emails() or
 * jmap_query_emails(). Never pass it a pointer from
 * jmap_sync_created() or jmap_sync_updated(): those are borrows into
 * the sync handle, released by jmap_sync_free().
 */
 ...
 * makes it a field the server always sets, defaulting to "", so it is
 * never NULL for a live handle.
 */
 ...
 * From address. When the server sends "from": [] you cannot tell it
 * apart from a missing from field: both return NULL.
 */
 ...
 *
 * You get JMAP_E_MISUSE for the setter of the wrong type, a NULL
 * string, and a value the library does not know. A query has no error
 * slot of its own, so the status a setter returns is the only report
 * you get about it.
 *
 * The setters check the value straight away, not when the query runs.
 * A value of the right type that is not legal in JMAP, such as a
 * malformed mailbox id, returns JMAP_E_VALIDATION and leaves the query
 * as it was.
 *
 ...
 * query together. JMAP_Q_LIMIT and JMAP_Q_SORT sit in slots of their
 * own.
 */
 ...
 * default paging.
 */
jmap_status jmap_query_emails(jmap_client *client, const char *account_id,
                              const jmap_query *query, jmap_emails **out);
 ...
 * out of range. th is itself a borrow into the threads handle, and so
 * is this string. Both live until jmap_threads_free().
 */
 ...
 */
jmap_status jmap_get_identities(jmap_client *client, const char *account_id,
                                jmap_identities **out);
 ...
 *
 * The first three reject an empty array and duplicate ids with
 ...
 * nothing, and duplicate ids go to the server as given, which answers
 * each one on its own (RFC 8620 section 5.3).
 *
 * The updated, destroyed and failure lists follow the order of the
 * server's own answer, not the order you submitted the ids in, and
 * that order can differ from one call to the next. To line results up
 * with the array you submitted, match by id. Never match by position.
 */
jmap_status jmap_mark_emails_read(jmap_client *client, const char *account_id,
                                  const char *const *ids, size_t n,
                                  jmap_set_result **out);
jmap_status jmap_mark_emails_unread(jmap_client *client, const char *account_id,
                                    const char *const *ids, size_t n,
                                    jmap_set_result **out);
/*
...
jmap_status jmap_move_emails(jmap_client *client, const char *account_id,
                             const char *const *ids, size_t n,
                             const char *mailbox_id, jmap_set_result **out);
 ...
 * jmap_set_result_free(). */
const char *jmap_set_result_failure_type_at(const jmap_set_result *r, size_t i);
 ...
 * record, and any other count returns JMAP_E_PROTOCOL. Must be freed
 * with jmap_vacation_free().
 */
jmap_status jmap_get_vacation(jmap_client *client, const char *account_id,
                              jmap_vacation **out);
void jmap_vacation_free(jmap_vacation *vacation);
/*
 * jmap_vacation_is_enabled() returns 1 when the vacation response is
 * on and 0 when it is off. JMAP_VACATION_DISABLED is 0 and
 ...
 * with jmap_vacation_update_free(). The three setters below fill it
 * in, one property each. The query options work the same way.
 *
 * A property that no setter names is left untouched. A setter called
 * with NULL clears that property to JSON null, which RFC 8621
 * section 8 allows for subject and textBody. Those are two different
 * requests, and only the setters can tell them apart. Calling a setter
 * twice replaces the earlier value.
 *
 * Each setter returns JMAP_OK or JMAP_E_MISUSE. You get JMAP_E_MISUSE
 * for a NULL handle, for a state that is neither
 * JMAP_VACATION_DISABLED nor JMAP_VACATION_ENABLED, and for a call
 * made before jmap_init(). A string value is never invalid on its own.
 * An update has no error slot of its own, so the status a setter
 * returns is the only report you get.
 *
 * This version has no setters for fromDate, toDate and htmlBody, and a
 * JMAP patch leaves a property it does not mention untouched. So
 * turning a response on through these setters keeps whatever dates the
 * record already holds, even dates another client wrote. RFC 8621
 * section 8 ties "effective immediately" and "effective indefinitely"
 * to a null fromDate and toDate on the record itself, so the response
 * is immediate and open-ended only when those are already null.
 */
 ...
 *
 * JMAP_OK means the call finished, not that the server accepted the
 * change. A refusal comes back as data (RFC 8620 section 5.3): the
 * usual refused update is JMAP_OK with
 ...
 * either list. The status does not promise that either holds anything.
 *
 * jmap_set_result_destroyed_count() is always 0 here, because this
 * update creates and destroys nothing.
 */
jmap_status jmap_set_vacation(jmap_client *client, const char *account_id,
...
/*
 * jmap_get_email_state() returns the current Email state string, which
 * is the since_state cursor your first jmap_sync_emails() call needs.
 *
 * On JMAP_OK the string is never NULL and never empty. A server that
 * claims an empty state fails the call with JMAP_E_PROTOCOL rather
 * than handing you a hollow value.
 *
 * When the call does not return JMAP_OK it leaves *out alone, so your
 * own starting value, usually NULL, survives to say "no cursor yet".
 * An empty string never stands in for it.
 *
 * The client owns the string. The next call that can fail on the same
 * client replaces it, exactly as with jmap_errmsg(). Copy it first if
 * it must outlive that call.
 */
jmap_status jmap_get_email_state(jmap_client *client, const char *account_id,
                                 const char **out);
 ...
 * jmap_sync_destroyed_at(), and destroyed wins: applying the update
 * afterwards would bring the record back.
 *
 ...
 */
jmap_status jmap_sync_emails(jmap_client *client, const char *account_id,
                             const char *since_state, jmap_sync **out);
void jmap_sync_free(jmap_sync *sync);

/*
 * jmap_sync_old_state() and jmap_sync_new_state() are never NULL and
 * never empty for a live handle: the library never carries an empty
 * state token past decoding. These borrows belong to the sync object,
 * not to the client, so they survive later calls on that client. They
 * live until jmap_sync_free().
 */
 ...
 * other read accessor here follows the same rule.
 */
 ...
 * free them.
 *
 * These emails carry metadata only, so jmap_email_text_body() returns
 * NULL for every one of them. To read the bodies, take their ids and
 * pass them to jmap_get_emails().
 */
 ...
 * name. Every option here is a value the send accepts; there is no
 * option for a field the send cannot carry.
 */
typedef enum {
  JMAP_MSG_IDENTITY_ID = 0,    /* string: id of the sending identity */
  JMAP_MSG_DRAFTS_MAILBOX = 1, /* string: mailbox the draft goes in */
  JMAP_MSG_SENT_MAILBOX = 2,   /* string: where it moves once sent */
  JMAP_MSG_FROM = 3,           /* string: the sender's address */
  JMAP_MSG_TO = 4,             /* string, holds a list: To recipient */
  JMAP_MSG_CC = 5,             /* string, holds a list: Cc recipient */
  JMAP_MSG_BCC = 6,            /* string, holds a list: Bcc recipient */
 ...
 * takes a string, so one setter covers them all.
 *
 ...
 * such as an id that is not an id, returns JMAP_E_VALIDATION and
 ...
 *
 * The library parses ids here but not addresses. A malformed address
 * returns JMAP_E_VALIDATION from jmap_send(), not from this call.
 *
 * A message has no error slot of its own, so the status this call
 * returns is the only report you get about it.
 * jmap_query_set_str() works the same way.
 */
jmap_status jmap_message_set_str(jmap_message *message, jmap_message_opt opt,
                                 const char *value);

/*
 * jmap_message_add_str() appends one address to an option that holds a
 * list. Set always replaces, add always appends.
 *
 * Only JMAP_MSG_TO, JMAP_MSG_CC and JMAP_MSG_BCC hold lists. Calling
 * this on any of the other six options returns JMAP_E_MISUSE, and so
 * do an option the library does not know, a NULL message and a NULL
 * value.
 *
 * You may append to an option that no setter has named yet. That
 * starts the list with this address.
 */
jmap_status jmap_message_add_str(jmap_message *message, jmap_message_opt opt,
                                 const char *value);
 ...
 * JMAP_MSG_DRAFTS_MAILBOX, JMAP_MSG_SENT_MAILBOX and JMAP_MSG_FROM
 * must all be set, and so must at least one of JMAP_MSG_TO,
 * JMAP_MSG_CC and JMAP_MSG_BCC. A message missing any of them returns
 * JMAP_E_MISUSE and nothing goes to the server. A message has no error
 * slot, so call jmap_errmsg() on the client to learn which option is
 * unset.
 *
 * When none of JMAP_MSG_TO, JMAP_MSG_CC and JMAP_MSG_BCC is set, the
 * option named in that message is JMAP_MSG_TO, standing for the whole
 * group. It does not mean JMAP_MSG_TO alone would satisfy the
 * requirement.
 *
 * JMAP_MSG_SUBJECT and JMAP_MSG_BODY are optional and default to
 * empty.
 *
...
jmap_status jmap_send(jmap_client *client, const char *account_id,
                      const jmap_message *message, jmap_send_result **out);
 ...
 * handle: a send that could not report both did not return JMAP_OK.
 * Both return NULL for a NULL handle, as every other read accessor
 * here does. The borrows live until jmap_send_result_free().
 */
