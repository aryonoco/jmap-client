...
#define JMAP_CLIENT_VERSION_PATCH 0
  ...
  int jmap_mailbox_has_right(const jmap_mailbox *mb,
                             jmap_mailbox_right r);
  ...
  /* Returns NULL when i is out of range. The borrow lives until
   * jmap_emails_free(). */
  const char *jmap_emails_notfound_at(const jmap_emails *emails,
                                      size_t i);
   ...
   */
  jmap_status jmap_get_threads(jmap_client *client,
                               const char *account_id,
  ...
  /* Returns NULL when i is out of range. The borrow lives until
   * jmap_threads_free(). */
  const jmap_thread *jmap_threads_at(const jmap_threads *threads,
                                     size_t i);
  ...
  const jmap_identity *jmap_identities_at(
      const jmap_identities *identities, size_t i);
   ...
   * other mailbox.
   */
  jmap_status jmap_move_emails(jmap_client *client,
                               const char *account_id,
                               const char *const *ids, size_t n,
                               const char *mailbox_id,
                               jmap_set_result **out);
  jmap_status jmap_destroy_emails(jmap_client *client,
                                  const char *account_id,
                                  const char *const *ids, size_t n,
                                  jmap_set_result **out);
  ...
  /* Returns NULL when i is out of range. The borrow lives until
   * jmap_set_result_free(). */
  const char *jmap_set_result_updated_at(const jmap_set_result *r,
                                         size_t i);
  size_t jmap_set_result_destroyed_count(const jmap_set_result *r);
  /* Returns NULL when i is out of range. The borrow lives until
   * jmap_set_result_free(). */
  const char *jmap_set_result_destroyed_at(const jmap_set_result *r,
                                           size_t i);
  size_t jmap_set_result_failure_count(const jmap_set_result *r);
  /* Returns NULL when i is out of range. The borrow lives until
   * jmap_set_result_free(). */
  const char *jmap_set_result_failure_id_at(const jmap_set_result *r,
                                            size_t i);
   ...
   */
  jmap_status jmap_get_vacation(jmap_client *client,
                                const char *account_id,
                                jmap_vacation **out);
  ...
  jmap_status jmap_vacation_update_set_enabled(
      jmap_vacation_update *update, jmap_vacation_state state);
  jmap_status jmap_vacation_update_set_subject(
      jmap_vacation_update *update, const char *subject);
  jmap_status jmap_vacation_update_set_text_body(
      jmap_vacation_update *update, const char *text_body);
  /*
