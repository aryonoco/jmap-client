...
static const char *SET_UPDATED_JSON =
    "{\"methodResponses\":[[\"Email/"
    "set\",{\"accountId\":\"A1\",\"oldState\":\"s1\",\"newState\":\"s2\","
    "\"updated\":{\"em-1\":null}},\"c0\"]],\"sessionState\":\"s1\"}";
static const char *SET_MIXED_JSON =
    "{\"methodResponses\":[[\"Email/"
    "set\",{\"accountId\":\"A1\",\"oldState\":\"s1\",\"newState\":\"s2\","
    "\"destroyed\":[\"em-1\"],\"notDestroyed\":{\"em-2\":{\"type\":"
    "\"forbidden\"}}},\"c0\"]],\"sessionState\":\"s1\"}";

int main(void) {
  assert(jmap_init() == JMAP_OK);
  /* mark_emails_read, mark_emails_unread and move_emails each drive
   * one Email/set exchange whose "updated" shape SET_UPDATED_JSON
   * already satisfies (its content does not depend on which patch
   * produced it); destroy gets its own partial-success fixture. */
  const char *bodies[] = {SESSION_JSON, SET_UPDATED_JSON, SET_UPDATED_JSON,
                          SET_UPDATED_JSON, SET_MIXED_JSON};
  ...
  assert(jmap_mark_emails_read(c, acct, ids, 1, &r) == JMAP_OK);
  ...
  assert(jmap_mark_emails_read(c, acct, ids, SIZE_MAX, &readHuge) ==
         JMAP_E_MISUSE);

  /* Mark unread: same shape, opposite patch value (keyword removal). */
  jmap_set_result *u = NULL;
  assert(jmap_mark_emails_unread(c, acct, ids, 1, &u) == JMAP_OK);
  ...
  assert(jmap_mark_emails_unread(c, acct, ids, SIZE_MAX, &unreadHuge) ==
         JMAP_E_MISUSE);
  ...
  assert(jmap_mark_emails_read(c, acct, NULL, 0, &e) == JMAP_E_VALIDATION);
